...
if($_GET["id_orderpay"]){
	//if($cookie->saltar == "si")
	$orderp = new Order($_GET["id_orderpay"]);
	if (Validate::isLoadedObject($orderp) AND $orderp->id_customer == $cookie->id_customer)
	{
		...
		$productsp = $orderp->getProducts();
		foreach($productsp as $prp){
			//print_r($prp);
			
			$idProduct = $prp["product_id"];
			$qty = $prp["product_quantity"];
			$idProductAttribute = $prp["product_attribute_id"];
			$customizationId = false;
			$cart->updateQty(intval($qty), intval($idProduct), intval($idProductAttribute), $customizationId, 'up');
		
		}
		
		//$cart = $cart->getCartByOrderId($_GET["id_orderpay"]);
		//$cart->OrderExists();
		//$cart->update();
		//$cookie->id_cart = intval($cart->id);
		...
		$cookie->__set("saltar","si");
		
		$cookie->__set("id_orden_antigua",$_GET["id_orderpay"]);
	}

}

//echo "<br>Saltar ->".$cookie->saltar;
...
function procesapxc(){
	...
	/*
	$orderp = new Order();
	$id_orderp = $orderp->getOrderByCartId($cart->id);
	$orderp = new Order($id_orderp);
	if (Validate::isLoadedObject($orderp) AND $orderp->id_customer == $cookie->id_customer)
	{
		$id_order_statep = intval($orderp->getCurrentState());
	
		if($id_order_statep==_PS_OS_PAYPAL_){
			$paso1=false;
			$paso2=false;
	...
	*/
	...
	if($paso1){
		if(!$cart->checkQuantitiesv2()){
			...
			$order->validateOrder(intval($cart->id), _PS_OS_PRESU_ , $total, Tools::displayError('Presupuesto por Fuera de Stock', false));
            
            $res = Order::updateProductsMissing($cart->id);
	...
	$total = floatval($cart->getOrderTotal(true, 3));
	$order->validateOrder(intval($cart->id), _PS_OS_PRESU_ , $total, Tools::displayError('Presupuesto', false));
	Tools::redirect('history.php');
