...
//					$this->_warnings[] = $category->image.' '.Tools::displayError('cannot be copied');
			...
			{
				$categoryAlreadyCreated = Category::searchByNameAndParent($defaultLanguageId, $category->name[$defaultLanguageId],$category->id_parent, true);
				if (empty($category->meta_title[$defaultLanguageId]))
				$category->meta_title[$defaultLanguageId]=$category->name[$defaultLanguageId];
				if (empty($category->meta_description[$defaultLanguageId]))
				$category->meta_description[$defaultLanguageId]=$category->name[$defaultLanguageId];
			
//print_r($category);
//print_r($categoryAlreadyCreated);
//echo "@@".$category->meta_title[$defaultLanguageId];
//$category->meta_title[$defaultLanguageId]="qwu";
//echo "@@".$category->meta_title[$defaultLanguageId];
//die("qq");
				// If category already in base, get id category back
				if ($categoryAlreadyCreated['id_category'] && ($categoryAlreadyCreated['id_parent']==$category->id_parent))
				{
...
echo "</pre>";
	...
	public function productImport()
	{
		...
		for ($current_line = 0; $line = fgetcsv($handle, MAX_LINE_SIZE, Tools::getValue('separator')); $current_line++)
		{
			...
			{
				$estaproducto=Product::getIdByReference($info['reference']);
				if (!empty($estaproducto))
					$info['id']=$estaproducto;
			}
//self::mostrar($info);
			if (array_key_exists('id', $info) AND intval($info['id']) AND Product::existsInDatabase(intval($info['id'])))
			{
				...
				if ($product->reduction_from == '0000-00-00' || $product->reduction_from =='')
					$product->reduction_from = date('Y-m-d');
				if ($product->reduction_to == '0000-00-00' || $product->reduction_to == '')
					$product->reduction_to = date('Y-m-d');
				$categoryData = Product::getIndexedCategories(intval($product->id));
//				self::mostrar($categoryData);
			...
			if (empty($info['category'])) unset($info['category']);
...
//			$id= Product::getIdByReference($product->reference);
//			if ($id)
//				$product->id=$id;
			// Find id_tax corresponding to given values for product taxe
			if (empty($product->quantity)) $product->quantity=1;
			if (empty($product->meta_title[$defaultLanguageId]))$product->meta_title[$defaultLanguageId]=$product->name[$defaultLanguageId]." ".$product->reference;
			if (empty($product->meta_description[$defaultLanguageId]))$product->meta_description[$defaultLanguageId]=$product->name[$defaultLanguageId]." ".$product->reference;
			if (empty($product->meta_keywords[$defaultLanguageId]))$product->meta_keywords[$defaultLanguageId]=$product->name[$defaultLanguageId].", ".$product->reference;
//			self::mostrar($product);die("qq");
				...
				foreach ($product->category AS $value)
				{
					...
					elseif (is_string($value) AND !empty($value))
					{
						...
						if (strpos($value,"~")>0)
						{
							$cats=split("~",$value);
							$idparent=0;
							$cats[0]=str_replace(chr(239),'',$cats[0]);
							$cats[0]=str_replace(chr(187),'',$cats[0]);
							$cats[0]=str_replace(chr(191),'',$cats[0]);
							$catparent=Category::searchByName($defaultLanguageId, $cats[0], true);
							//self::mostrar($catparent);
//						$cats[0]=utf8_decode ($cats[0]);

							if (empty($catparent))
							{
								$categoryToCreate= new Category();
								$categoryToCreate->name = self::createMultiLangField($cats[0]);
								$categoryToCreate->active = 1;
								$categoryToCreate->id_parent = 1; // Default parent is home for unkno
								if (($fieldError = $categoryToCreate->validateFields(UNFRIENDLY_ERROR, true)) === true AND ($langFieldError = $categoryToCreate->validateFieldsLang(UNFRIENDLY_ERROR, true)) === true AND $categoryToCreate->add()) {
								$catparent=$categoryToCreate;
								...
								$idparent=$catparent['id_category'];
							//self::mostrar($catparent);
							
							$category = Category::searchByNameAndParent($defaultLanguageId, $cats[1], $idparent, true);
							
							//self::mostrar($category);
							if (empty($category))
							{
							
								$categoryToCreate= new Category();
							$categoryToCreate->name = self::createMultiLangField($cats[1]);
							$categoryToCreate->active = 1;
							$categoryToCreate->id_parent = $idparent; // Default parent is home for unkno
									if (($fieldError = $categoryToCreate->validateFields(UNFRIENDLY_ERROR, true)) === true AND ($langFieldError = $categoryToCreate->validateFieldsLang(UNFRIENDLY_ERROR, true)) === true AND $categoryToCreate->add()) {
									$category=$categoryToCreate;
									$product->id_category[]=intval($category->id);
									}
									else
									{
									$this->_errors[] = $categoryToCreate->name[$defaultLanguageId].(isset($categoryToCreate->id) ? ' ('.$categoryToCreate->id.')' : '').' '.Tools::displayError('cannot be saved');
			...
			if (count($product->id_category)==0) $product->id_category[0]=1;
			...
			else
			{
				...
				if (isset($product->image) AND is_array($product->image) and sizeof($product->image))
				{
					...
					if ($productHasImages && !empty($product->image[0]))
				...
				if (isset($product->id_category))
					$product->updateCategories(array_map('intval', $product->id_category),true);
	...
	public function displayForm($isMainTab = true)
	{
		...
		if ($i==1 || $i==0)
		{
			...
			if (Tools::getValue('entity') == $i)
				...
				if (isset($_GET['identity']) && $_GET['identity']== $i)
				echo ' selected="selected" ';
			echo'>'.$entity.'</option>';
			}
		}
	...
	public function displayCSV()
	{
		...
		<form action="'.$currentIndex.'&token='.$this->token.'" method="post" id="import_form" name="import_form">
			'.' <input type="hidden" size="2" name="skip" value="0" /> '.'
		...
		echo '
			<table>
				<tr>
					<td valign="top" align="center"><input id="btn_left" value="<<" type="button" class="button" onclick="showTable(current - 1)" /></td>
					<td align="left">';
	...
	public function postProcess()
	{
		...
		if (Tools::isSubmit('submitFileUpload'))
		{
			...
			elseif (!file_exists($_FILES['file']['tmp_name']) OR !@rename($_FILES['file']['tmp_name'], dirname(__FILE__).'/../import/'.$_FILES['file']['name'].'.'.date('Ymdhis')))
				...
				Tools::redirectAdmin($currentIndex.(isset($_GET['identity'])?'&identity='.$_GET['identity']:'').'&token='.Tools::getValue('token').'&conf=18');
		}
