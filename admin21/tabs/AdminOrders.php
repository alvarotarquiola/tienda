		...
		elseif (Tools::isSubmit('submitState') AND ($id_order = intval(Tools::getValue('id_order'))) AND Validate::isLoadedObject($order = new Order($id_order)))
		{
			...
			if ($this->tabAccess['edit'] === '1')
			{
				...
				else
				{
                    ...
                    $result_state = OrderHistory::upadateQuantityProducto($id_order, $newOrderStatusId);
	...
	public function viewDetails()
	{
		...
		*/
		...
		if ($order->total_paid != $order->total_paid_real)
			echo '<center><span class="warning" style="font-size: 16px; display:none;">'.$this->l('Warning:').' '.Tools::displayPrice($order->total_paid_real, $currency, false, false).' '.$this->l('paid instead of').' '.Tools::displayPrice($order->total_paid, $currency, false, false).' !</span></center><div class="clear"><br /><br /></div>';
		...
		if($order->isduplicated($order->id)){
			echo "<form><fieldset>";
			//echo "<legend>Orden Duplicada</legend>";
            echo "<legend>".$this->l('Duplicate orders')."</legend>";
			
			$duplicadas = $order->getduplicateds($order->id);
			//echo "<br>Copia de Ordenes: <ul>";
            echo "<br>".$this->l('Copy of orders').": <ul>";
			$cuenta=0;
			foreach($duplicadas as $dupli){
				if($cuenta==0){
					//$original = " (Original)";
				...
				$llink = "index.php?tab=AdminOrders&id_order=".$dupli["id_order"]."&vieworder&token=".$_GET["token"];
				echo "<li><a href='".$llink."'> - ".$this->l('View Order')." ".$dupli["id_order"].$original."</a></li>";
			}
		...
		<fieldset style="width: 400px">
            ...
            <div style="overflow: hidden;">
                <label style="text-align:left;width:auto;">'.$this->l('Original cart:').' </label>
    			<div style="margin:3px 0 1em 5px;float:left;"><a href="?tab=AdminCarts&id_cart='.$cart->id.'&viewcart&token='.Tools::getAdminToken('AdminCarts'.intval(Tab::getIdFromClassName('AdminCarts')).intval($cookie->id_employee)).'">'.$this->l('Cart #').sprintf('%06d', $cart->id).'</a></div>
            </div>
            <div style="overflow: hidden;">
                <label style="text-align:left;width:auto;">'.$this->l('Payment mode:').' </label>
    			<div style="margin:3px 0 1em 5px;float:left;">'.$order->payment.' '.($order->module ? '('.$order->module.')' : '').'</div>
            </div>
				...
				<div style="float:left;">
					<table style="width: 868px;" cellspacing="0" cellpadding="0" class="table" id="orderProducts">
						<tr>
							...
							//if(!$order->isduplicated($order->id)){
							//echo '<th><a href="'.$_SERVER['REQUEST_URI'].'&duplicar=si">Duplicar</a></th>';
								//echo '<th> Duplicar ? </th>';
                                echo '<th> '.$this->l('Duplicate ?').' </th>';
							//}
		echo '
						</tr>';
						...
						foreach ($products as $k => $product)
						{
							...
							if ($product['product_quantity'] > $product['customizationQuantityTotal'])
							{
								...
								if ((!$order->hasBeenDelivered() OR Configuration::get('PS_ORDER_RETURN')) AND intval($product['product_quantity_return']) < intval($product['product_quantity']))
									...
									echo '<input type="checkbox" name="id_order_detail['.$k.']" id="id_order_detail['.$k.']" value="'.$product['id_order_detail'].'" />';
								else
									echo '--';
								echo '
									</td>
				...
				if (!$order->hasBeenDelivered() OR ($order->hasBeenDelivered() AND Configuration::get('PS_ORDER_RETURN')))
					echo '
                    <div style="text-align:center; margin-top:5px;"><input type="submit" name="cancelProduct" value="'.($order->hasBeenDelivered() ? $this->l('Return products') : ($order->hasBeenPaid() ? $this->l('Refund products') : $this->l('Delete and duplicate products'))).'" class="button" style="margin-top:8px;" /></div>';
				if(!$order->isduplicated($order->id)){	
					//echo '<div style="text-align:center; margin-top:5px;"><input type="submit" name="duplicar" value="Duplicar" class="button" style="margin-top:8px;"></div>';
					}
				echo '
				</div>';
			echo '
