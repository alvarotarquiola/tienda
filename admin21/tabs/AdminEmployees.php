	...
	public function displayForm($isMainTab = true)
	{
		...
		'.($obj->id ? '<input type="hidden" name="id_'.$this->table.'" value="'.$obj->id.'" />' : '').'
			<fieldset><legend><img src="../img/admin/nav-user.gif" />'.$this->l('Employees').'</legend>
				...
				<div class="margin-form">
					<input type="radio" name="active" id="active_on" disabled value="1" '.($this->getFieldValue($obj, 'active') ? 'checked="checked" ' : '').'/>
					<label class="t" for="active_on"> <img src="../img/admin/enabled.gif" alt="'.$this->l('Enabled').'" title="'.$this->l('Enabled').'" /></label>
					<input type="radio" name="active" id="active_off" disabled value="0" '.(!$this->getFieldValue($obj, 'active') ? 'checked="checked" ' : '').'/>
