	...
	public function displayForm($isMainTab = true)
	{
		...
		if(is_writable(PS_ADMIN_DIR.'/backups/'))
		{
            if($enable_shop == 1){
    			...
    			if ($object->add())
    			{
    				echo '<div class="conf confirm"><img src="../img/admin/ok.gif" />&nbsp;'.$this->l('Back-up Creation successful').' !';
	...
	*/
	public function viewbackup()
	{
		...
		if ($object->id)
		{
			...
			echo '<div class="conf confirm"><img src="../img/admin/ok.gif" />&nbsp;'.$this->l('Beginning download ...');
			echo '<br />'.$this->l('Back-up file should automatically download.');
			echo '<br />'.$this->l('If not,').' <b><a href="'.$url.'">'.$this->l('please click here!').'</a></b>';
