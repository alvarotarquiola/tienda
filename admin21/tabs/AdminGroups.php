        ...
        echo '
            <script type="text/javascript">
				$(document).ready(function() {
                    jQuery("#form_groups").find(".text_group_form input").blur(function(){
                       var textlang = jQuery(this).attr("value");
                       $(".text_group_form2").each(function(index){
		...
		'.($obj->id ? '<input type="hidden" name="id_'.$this->table.'" value="'.$obj->id.'" />' : '').'
			<fieldset><legend><img src="../img/admin/tab-groups.gif" />'.$this->l('Group').'</legend>
				...
				foreach ($this->_languages as $language)
					echo '
					<div id="name_'.$language['id_lang'].'" class="'.($language['id_lang'] == $this->_defaultFormLanguage ? 'text_group_form' : 'text_group_form2').'" style="display: '.($language['id_lang'] == $this->_defaultFormLanguage ? 'block' : 'none').'; float: left;">
