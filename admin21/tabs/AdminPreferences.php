...
class AdminPreferences extends AdminTab
{
	...
	protected function _displayForm($name, $fields, $tabname, $size, $icon)
	{
		...
		foreach ($fields AS $key => $field)
		{
            ...
            //if(($key == "PS_BASE_URI") || ($key == "PS_SHOP_ENABLE")){
    			...
    			if (!in_array($field['type'], array('image', 'radio', 'container', 'container_end')) OR isset($field['show']))
    				echo '<div id="'.$key.'" class="'.$key.'" style="clear: both; padding-top:15px; display: '.(($key == 'PS_BASE_URI' || $key == 'PS_MAINTENANCE_IP' || $key == 'PS_SSL_ENABLED' || $key == 'PS_TOKEN_ENABLE' || $key == 'PS_REWRITING_SETTINGS' || $key == 'PS_HELPBOX' || $key == 'PS_CONDITIONS' || $key == 'PS_GIFT_WRAPPING' || $key == 'PS_GIFT_WRAPPING_PRICE' || $key == 'PS_GIFT_WRAPPING_TAX' || $key == 'PS_RECYCLABLE_PACK' || $key == 'PS_CART_FOLLOWING' || $key == 'PS_PRICE_ROUND_MODE' || $key == 'PRESTASTORE_LIVE' || $key == 'PS_TIMEZONE' || $key == 'PS_THEME_V11') ? 'none' : 'block').';">'.($field['title'] ? '<label >'.$field['title'].'</label>' : '').'<div class="margin-form" style="padding-top:5px;">';
		...
		echo '	<div align="center" style="margin-top: 20px;">
					<input type="submit" value="'.$this->l('   Save   ', 'AdminPreferences').'" name="submit'.ucfirst($name).$this->table.'" class="button action_submit_form" />
				</div>
    ...
    public function displayFormBackuoRestore()
    {
        global $currentIndex;
        
        $backuprestore = $_GET["backuprestore"];
        $enable_backuprestore = Configuration::get("PS_BACKUP_RESTORE");
        
        
        echo '
        <script type="text/javascript">
            jQuery(function(){
                jQuery("#form_backup_restore").submit(function(){
                    var ele = jQuery(this);
                    ...
                    jQuery.ajax({
                        ...
                        success: function(res){
                            if(res == "success"){
                                jQuery("#message_success_backup_restore").css("display", "block");
        ...
        echo '
            <div id="message_success_backup_restore" class="message_result" style="display: none; width: 400px;">
                <img src="../img/admin/ok.gif" alt="">'.$this->l("Settings update successful").'
            </div>
        ';

        echo '
            <form id="form_backup_restore" action="ajax.php" method="post" class="width2" enctype="multipart/form-data" style="width:520px" onsubmit="return false;">
            
                <fieldset>
                    <div class="PS_BACKUP_RESTORE" style="clear: both; display: block; overflow: hidden;">
                        <label style="width: 350px;">'.$this->l('Enables Backup and Restore with Active Shop:').'</label>
                        <div class="margin-form" style="float:left; margin-left:5px; padding:3px 0 0 0;">
                            <label for="PS_BACKUP_RESTORE_on" class="t">
                                <img title="Sí" alt="Sí" src="../img/admin/enabled.gif">
                            </label>
                            <input type="radio" "'.($enable_backuprestore ? ' checked="checked"' : '').'" value="1" id="PS_BACKUP_RESTORE_on" name="PS_BACKUP_RESTORE">
                            <label class="t" for="PS_BACKUP_RESTORE_off"> '.$this->l('Yes').'</label>
                            <label class="t" for="PS_BACKUP_RESTORE_off">
                                <img style="margin-left: 10px;" title="No" alt="No" src="../img/admin/disabled.gif">
                            </label>
                            <input type="radio" "'.(!$enable_backuprestore ? ' checked="checked"' : '').'" value="0" id="PS_BACKUP_RESTORE_off" name="PS_BACKUP_RESTORE">
                            <label for="PS_BACKUP_RESTORE_off" class="t"> '.$this->l('No').'</label>
                        </div>
                    </div>
                    <input type="hidden" id="backup_restore" name="backup_restore" value="backuprestore">
                    <div style="text-align: center; margin: 20px 10px 0 0;">
    					<input type="submit" id="btn_backup_restore" class="button" name="savebackuprestore" value="'.$this->l('   Save   ').'">
    				</div>
                </fieldset>
            </form>
        ';
    ...
    public function showOnlineVisitors($moduleName = "statslive")
    {
        global $cookie;
