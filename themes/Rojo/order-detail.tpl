...
<p class="bold">{l s='Peso Total:'} <span class="color-myaccount">{$total_peso}</span></p>
<p class="bold">{l s='Total Productos:'} <span class="color-myaccount">{$total_products}</span></p>
...
{if $pxc}
	...
	<a href="{$base_dir}order.php?step=0&id_orderpay={$order->id|intval}">{l s='Pagar Orden'}</a>
	</div>
{/if}
