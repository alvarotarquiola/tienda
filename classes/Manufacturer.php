...
class		Manufacturer extends ObjectModel
{
	...
	*/	
    ...
    static public function checkExcelManufacturer($ordenado)
    {
        ...
        for($linea = 2; $linea < $tama; $linea++)
        {
            foreach($ordenado[$linea] as $key => $value)
            {
                ...
                if($value != "")
                {
                    ...
                    }elseif($key == "Manufacture-description-small-lang-1"){
                        if($cantChart > 128)
                            return array("message" => "error", "column" => $key);    
                    }elseif($key == "Manufacture-description-small-lang-2"){
                        if($cantChart > 128)
                            return array("message" => "error", "column" => $key);   
                    }elseif($key == "Manufacture-description-small-lang-3"){
                        if($cantChart > 128)
                            return array("message" => "error", "column" => $key);    
                    }elseif($key == "Manufacture-description-small-lang-4"){
                        if($cantChart > 128)
                            return array("message" => "error", "column" => $key);    
                    }elseif($key == "Manufacture-description-small-lang-5"){
                        if($cantChart > 128)
                            return array("message" => "error", "column" => $key);    
                    }elseif($key == "Manufacture-image"){
    ...
    static public function getIdByManufactureLang($id_manufacturer, $id_lang)
    {
        $query = Db::getInstance()->ExecuteS('
		SELECT *
		FROM '._DB_PREFIX_.'manufacturer_lang
		WHERE `id_manufacturer` = '.intval($id_manufacturer).' AND `id_lang` = '.$id_lang.' LIMIT 1');
        
        return $query[0];
    }
    
    static public function existeManufacturerLang($id_manufacturer, $id_lang)
    {
        $manufactureLang = Manufacturer::getIdByManufactureLang($id_manufacturer, $id_lang);
        $res = false;

        if(count($manufactureLang) > 0){
            $res = true;
        }
        return $res;
    }
