...
class		Category extends ObjectModel
{
	...
	public function getFields()
	{
        ...
        $fields['position_category'] = intval($this->position_category);
	...
	public	function add($autodate = true, $nullValues = false)
	{
        ...
        $categoriesLan = Category::getCopyName($this->id);
        $metas = Category::setChangeMetas($this->id);
	...
	static public function searchByNameAndParent($id_lang, $query, $idparent, $unrestricted = false)
	{
		if ($unrestricted === true)
			return Db::getInstance()->getRow('
			SELECT c.*, cl.*
			FROM `'._DB_PREFIX_.'category` c
			LEFT JOIN `'._DB_PREFIX_.'category_lang` cl ON (c.`id_category` = cl.`id_category`)
			WHERE id_parent=\''.$idparent.'\' and `name` LIKE \''.pSQL($query).'\'');
		else
			return Db::getInstance()->ExecuteS('
			SELECT c.*, cl.*
			FROM `'._DB_PREFIX_.'category` c
			LEFT JOIN `'._DB_PREFIX_.'category_lang` cl ON (c.`id_category` = cl.`id_category` AND `id_lang` = '.intval($id_lang).')
			WHERE id_parent=\''.$idparent.'\' and `name` LIKE \'%'.pSQL($query).'%\' AND c.`id_category` != 1');
	}
	/**
	...
	*/
    ...
    static public function getCategoriesLang($id_category)
    {
    ...
    static public function getCopyName($id_categoria)
    {
        $categoriesLang = Category::getCategoriesLang($id_categoria);
        foreach($categoriesLang as $categoryLang):
            if($categoryLang["name"] != ""){
                $new_name = "";
                $aux_array = explode(".", $categoryLang["name"]);
                ...
                Category::updateName2Lang($id_categoria, $categoryLang["id_lang"], $new_name);
            }
        endforeach;
    }
    
    /*static public function getCopyNamePositionCategory($id_categoria)
    {
        $categoriesLang = Category::getCategoriesLang($id_categoria);
        $category = 
        foreach($categoriesLang as $categoryLang):
    ...
    static public function updateName2Lang($id_category, $id_lang, $name)
    {   
        $field["name_2"] = $name;
        
        Db::getInstance()->AutoExecute(_DB_PREFIX_.'category_lang', $field, 'UPDATE', '`id_category` = '.intval($id_category).' AND `id_lang` = '.intval($id_lang));
    }
    
    static public function updateCategoryLang($id_category, $id_lang, $field)
    {   
        Db::getInstance()->AutoExecute(_DB_PREFIX_.'category_lang', $field, 'UPDATE', '`id_category` = '.intval($id_category).' AND `id_lang` = '.intval($id_lang));
    }
    
    static public function setChangeMetas($id_category)
    {
        $categoriesLang = Category::getCategoriesLang($id_category);
        
        foreach($categoriesLang as $categoryLang):
            if($categoryLang["name"] != ""):
                $data["meta_title"] = strtolower($categoryLang["name_2"]);
                $data["meta_keywords"] = strtolower($categoryLang["name_2"]);
                $data["meta_description"] = $categoryLang["description"];
                Category::updateCategoryLang($id_category, $categoryLang["id_lang"], $data);
            endif;
        endforeach;
    }
    
    static public function validCharacterDatas($type)
    {
        if($type == "category")
        {
            $datas = array("Cat-Ref", "1-Cat-order", "1-Cat-lang-1", "1-Cat-lang-2", "1-Cat-lang-3", "1-Cat-lang-4", "1-Cat-lang-5", 
                              "2-Cat-order", "2-Cat-lang-1", "2-Cat-lang-2", "2-Cat-lang-3", "2-Cat-lang-4", "2-Cat-lang-5",
                              "3-Cat-order", "3-Cat-lang-1", "3-Cat-lang-2", "3-Cat-lang-3", "3-Cat-lang-4", "3-Cat-lang-5",
                              "Cat-description-lang-1", "Cat-description-lang-2", "Cat-description-lang-3", "Cat-description-lang-4", "Cat-description-lang-5",
                              "Cat-active", "Cat-image");
        }
        if($type == "manufacturer")
        {
            $datas = array("Manufacture", "Manufacture-description-small-lang-1", "Manufacture-description-small-lang-2", "Manufacture-description-small-lang-3", 
                              "Manufacture-description-small-lang-4", "Manufacture-description-small-lang-5", "Manufacture-description-lang-1", 
                              "Manufacture-description-lang-2", "Manufacture-description-lang-3", "Manufacture-description-lang-4",
                              "Manufacture-description-lang-5", "Manufacture-image");
        }
        if($type == "product")
        {
            $datas = array("Order", "Ref", "Name-lang-1", "Name-lang-2", "Name-lang-3", "Name-lang-4", "Name-lang-5", 
                              "1-Cat", "2-Cat", "3-Cat", "Active", "Price-without-TAX", "TAX", "Description-small-lang-1",
                              "Description-small-lang-2", "Description-small-lang-3", "Description-small-lang-4", "Description-small-lang-5",
                              "Ecotax", "Sale", "Sale-amount", "Sale-%", "Sale-from", "Sale-up", "Weight", "Stock", 
                              "Text-available-lang-1", "Text-available-lang-2", "Text-available-lang-3", "Text-available-lang-4", "Text-available-lang-5", 
                              "Text-off-stock-lang-1", "Text-off-stock-lang-2", "Text-off-stock-lang-3", "Text-off-stock-lang-4", "Text-off-stock-lang-5",
                              "Manufecture", "Supplier", "Ref-supplier");
        }
        if($type == "accesory")
        {
            $datas = array("Ref", "Accessories");
        }
        
        return $datas;
    }
    
    static public function checkExcelCategory($ordenado)
    {
        ...
        for($linea = 2; $linea < $tama; $linea++)
        {
            foreach($ordenado[$linea] as $key => $value)
            {
                ...
                if($value != "")
                {
                    if($key == "1-Cat-order")
                    {
                        if(!$is_numeric || $cantChart > 10)
                            return array("message" => "error", "column" => $key);  
                    }elseif($key == "1-Cat-lang-1"){
                        if($cantChart > 128)
                            return array("message" => "error", "column" => $key);
                    }elseif($key == "1-Cat-lang-2"){
                        if($cantChart > 128)
                            return array("message" => "error", "column" => $key);    
                    }elseif($key == "1-Cat-lang-3"){
                        if($cantChart > 128)
                            return array("message" => "error", "column" => $key);   
                    }elseif($key == "1-Cat-lang-4"){
                        if($cantChart > 128)
                            return array("",$key);    
                    }elseif($key == "1-Cat-lang-5"){
                        if($cantChart > 128)
                            return array("message" => "error", "column" => $key);    
                    }elseif($key == "2-Cat-order"){
                        if(!$is_numeric || $cantChart > 10)
                            return array("message" => "error", "column" => $key);    
                    }elseif($key == "2-Cat-lang-1"){
                        if($cantChart > 128)
                            return array("message" => "error", "column" => $key);    
                    }elseif($key == "2-Cat-lang-2"){
                        if($cantChart > 128)
                            return array("message" => "error", "column" => $key);    
                    }elseif($key == "2-Cat-lang-3"){
                        if($cantChart > 128)
                            return array("message" => "error", "column" => $key);    
                    }elseif($key == "2-Cat-lang-4"){
                        if($cantChart > 128)
                            return array("message" => "error", "column" => $key);   
                    }elseif($key == "2-Cat-lang-5"){
                        if($cantChart > 128)
                            return array("message" => "error", "column" => $key);    
                    }elseif($key == "3-Cat-order"){
                        if(!$is_numeric || $cantChart > 10)
                            return array("message" => "error", "column" => $key);    
                    }elseif($key == "3-Cat-lang-1"){
                        if($cantChart > 128)
                            return array("message" => "error", "column" => $key);    
                    }elseif($key == "3-Cat-lang-2"){
                        if($cantChart > 128)
                            return array("message" => "error", "column" => $key);    
                    }elseif($key == "3-Cat-lang-3"){
                        if($cantChart > 128)
                            return array("message" => "error", "column" => $key);    
                    }elseif($key == "3-Cat-lang-4"){
                        if($cantChart > 128)
                            return array("message" => "error", "column" => $key);    
                    }elseif($key == "3-Cat-lang-5"){
                        if($cantChart > 128)
                            return array("message" => "error", "column" => $key);    
                    }elseif($key == "Cat-active"){
                        if($value < 0 || $value > 1)
                            return array("message" => "error", "column" => $key);   
                    }elseif($key == "Cat-image"){
                        $peso_array = ImageAttachments::verifyZiseFile($value);
                        if($peso_array[1] == "KB"){
                            if($peso_array[0] > 300)
                                return array("message" => "error", "column" => $key);
                        }
    ...
    static public function getCategoryLanguages($id_category)
    {
        return Db::getInstance()->ExecuteS('
            SELECT *
            FROM `'._DB_PREFIX_.'category_lang`
    ...
    static public function getLanguageCategories($type = 1)
    {
        ...
        if($type == 1)
        {
            ...
            foreach($allCategories as $categoria):
                ...
                if($categoria["id_category"] != 1):
                    ...
                    $ids_cate2 = ExportReports::getIdsCategoria($allCategories[$i+1]['id_category'], $allCategories[$i+1]['id_parent'], $allCategories[$i+1]['level_depth']);
                    $aux_cate = ExportReports::getCategoriesLang($ids_cate[0], 1);
                    $aux_cate2 = ExportReports::getCategoriesLang($ids_cate2[0], 1);
                    /*echo "<pre>";
                    print_r($allCategories[$i]['id_category']);
                    echo "</pre>";
                    echo "<pre>";
                    print_r($aux_cate);
                    echo "</pre>";
                    ...
                    if(count($ids_cate) == 1){
                        ...
                        //$valid_array = array_merge(array($order_1),$cates_lang[0], array(""), $cates_lang2, array(""), $cates_lang3, $cates_lang[1], array($categoria["active"]));
                        
                        $valid_array = array_merge(array($cat_ref),array($order_1),$cates_lang[0]);
            ...
            foreach($allCategories as $categoria):
                ...
                if($categoria["id_category"] != 1):
                    ...
                    if(count($ids_cate) == 2){
                        ...
                        $valid_array = array_merge(array($cat_ref),array(""),$cates_lang, array($order_2), $cates_lang2[0]);   
                    }
                    if(count($valid_array) > 0)
                        $categories[] = $valid_array;
                endif; 
            ...
            foreach($allCategories as $categoria):
                ...
                if($categoria["id_category"] != 1):
                    ...
                    /*if(count($ids_cate) == 1){
                        ...
                        $cates_lang3 = ExportReports::nameEmpty();
                        $order_2 = !empty($categoria["position_category"])?$categoria["position_category"]:"";
                        $valid_array = array_merge(array(""),$cates_lang, array($order_2), $cates_lang2[0], array(""),$cates_lang3, $cates_lang2[1], array($categoria["active"]));   
                    }else*/if(count($ids_cate) == 3){
                        $cates_lang = ExportReports::getCategoriesLang($ids_cate[0],2);
                        ...
                        $valid_array = array_merge(array($cat_ref),array(""),$cates_lang, array(""),$cates_lang2, array($order_3),$cates_lang3[0]);
                    }
                    if(count($valid_array) > 0)
                        $categories[] = $valid_array;
                endif; 
