...
class		Order extends ObjectModel
{
	...
	*/
	 ...
	 public function copyorder($id_order){
		...
		foreach($resulta as $key => $val){
			...
			if($key=="id_order"){
				...
				$sqlvals .=", '".date("Y-m-d H:i:s")."'";
			}elseif($key=="date_upd"){
				$sqlvals .=", '".date("Y-m-d H:i:s")."'";	
			}elseif($key=="invoice_date"){
				$sqlvals .=", '".date("Y-m-d H:i:s")."'";
				//$sqlvals .=", NULL";
			}elseif($key=="invoice_number"){
				$sqlvals .=", '0'";
			}elseif($key=="delivery_number"){
				$sqlvals .=", '0'";
		...
		`id_order_detail` ,`id_order` ,`product_id` ,`product_attribute_id` ,`product_name` ,`product_quantity` ,
		`product_quantity_in_stock` ,`product_quantity_refunded` ,`product_quantity_return` ,`product_quantity_reinjected` ,
		`product_price` ,`product_quantity_discount` ,`product_ean13` ,`product_reference` ,`product_supplier_reference` ,
		`product_weight` ,`tax_name` ,`tax_rate` ,`ecotax` ,`discount_quantity_applied` ,`download_hash` ,
		`download_nb` ,`download_deadline`
		)
		VALUES (";
	...
	public function getduplicateds($id_order){
		$sql = '
		SELECT id_cart 
		FROM `'._DB_PREFIX_.'orders`
		WHERE id_order='.$id_order.'
    ...
    static public function getOrderByCart($id_cart)
	{
		return Db::getInstance()->getRow('
		SELECT *
		FROM `'._DB_PREFIX_.'orders`
    ...
    static public function updateProductsMissing($id_cart)
    {
        $order_aux = Order::getOrderByCart($id_cart);
        $order_detail = OrderDetail::getByIdOrder($order_aux["id_order"]);
        $product_aux = new Product($order_detail["product_id"]);
        ...
        $quantity_messing = $product_aux->quantity_missing;
        
        if($product_quantity > 0 && $product_quantity_in_stock == 0){
            $res_quantity = ($product_aux->quantity_missing) + $product_quantity;
            $ressult = Product::updateProductQuantityMissing($product_aux->id, $res_quantity);
        }elseif($product_quantity > 0 && $product_quantity_in_stock > 0){
            $res_quantity = $product_quantity - $product_quantity_in_stock;
            $res_quantity_2 = ($product_aux->quantity_missing) + $res_quantity;
            $ressult = Product::updateProductQuantityMissing($product_aux->id, $res_quantity_2);
        }
