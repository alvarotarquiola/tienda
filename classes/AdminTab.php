...
abstract class AdminTab
{
    ...
    function translations($string, $class = __CLASS__, $addslashes = FALSE, $htmlentities = TRUE)
	{
		global $_LANGADM;
    ...
    public function hidesFields()
    {
        echo '
            <script type="text/javascript">
                jQuery(function(){
                    jQuery(".table tr th.customer_newsletter, .table tr td.customer_Filter_newsletter").css("display", "none");
                    jQuery(".table tr td.customer_newsletter").css("display", "none");
                });
            </script>
        ';
    }
			...
			if ((isset($ok_inc) AND $ok_inc) OR !sizeof($actions))
			{
				...
				if (!sizeof($actions))
					if (($methodname == 'displayErrors' AND sizeof($adminTab->_errors)) OR $methodname != 'displayErrors')
						echo (isset($this->_includeTabTitle[$key]) ? '<h2 class="title_key_access">'.$this->_includeTabTitle[$key].'</h2>' : '');
	...
	public function postProcess()
	{
		...
		elseif (isset($_GET['delete'.$this->table]))
		{
            ...
            $messages = $this->deleteMessageOrder($object->id);
			...
			if ($this->tabAccess['delete'] === '1')
			{
				if (Validate::isLoadedObject($object = $this->loadObject()) AND isset($this->fieldImageSettings))
				{
					...
					else
					{
                        ...
                        $this->deleteMessageOrder($object->id);
						if ($this->deleted)
						{
							$object->deleted = 1;
							if ($object->update())
	...
	public function displayListHeader($token = NULL)
	{
		...
		echo '#'.$this->table.'" class="form '.$this->table.'_form">
		...
		echo '
					...
					<span style="float: right;">
						<input type="submit" name="submitReset'.$this->table.'" value="'.$this->l('Reset').'" class="button action_btn_reset" />
						<input type="submit" id="submitFilterButton_'.$this->table.'" name="submitFilter" value="'.$this->l('Filter').'" class="button action_btn_filter" />
					</span>
		...
		{
            //echo $key;
			echo '	<th '.(isset($params['widthColumn']) ? 'style="width: '.$params['widthColumn'].'px"' : '').' class="'.$this->table.'_'.$key.'">'.$params['title'];
		...
		foreach ($this->fieldsDisplay AS $key => $params)
		{
			...
			echo '<td'.(isset($params['align']) ? ' class="'.$this->table.'_Filter_'.$key.' '.$params['align'].'"' : '').'>';
	...
	public function displayList()
	{
        ...
        if ($this->edit AND (!isset($this->noAdd) OR !$this->noAdd)){
            //if($this->table != "address"){
                echo '<br /><a class="link_add_'.$this->table.' action_add_new" href="'.$currentIndex.'&add'.$this->table.'&token='.$this->token.'"><img src="../img/admin/add.gif" border="0" /> '.$this->l('Add new').'</a><br /><br />'; 
            //}
        }
			
            
        //Se agrega esta lienea porque no se crea el enlace nuevo cms.
        if($this->table == "cms"){
            echo '<br /><a class="link_add_cms_'.$this->table.' action_add_new" href="'.$currentIndex.'&add'.$this->table.'&token='.$this->token.'"><img src="../img/admin/add.gif" border="0" /> '.$this->l('Add new').'</a><br /><br />';
        }
        ...
        $this->displayFormRemote($this->token, $this->table);
	}
	...
	public function displayListContent($token = NULL)
	{
		...
		if ($this->_list)
		{
			foreach ($this->_list AS $i => $tr)
			{
				...
				foreach ($this->fieldsDisplay AS $key => $params)
				{
					...
					echo '
					<td '.(isset($params['position']) ? ' id="td_'.(isset($id_category) AND $id_category ? $id_category : 0).'_'.$id.'"' : '').' class="pointer'.((isset($params['position']) AND $this->_orderBy == 'position')? ' dragHandle' : ''). (isset($params['align']) ? ' '.$params['align'] : '').' '.$this->table.'_'.$key.'" ';
				...
				if ($this->edit OR $this->delete OR ($this->view AND $this->view !== 'noActionColumn'))
				{
					...
					if ($this->view)
						echo '
						<a class="action_view" href="'.$currentIndex.'&'.$this->identifier.'='.$id.'&view'.$this->table.'&token='.($token!=NULL ? $token : $this->token).'">
						<img src="../img/admin/details.gif" alt="'.$_cacheLang['View'].'" title="'.$_cacheLang['View'].'" /></a>';
					if ($this->edit)
						echo '
						<a class="action_edit" href="'.$currentIndex.'&'.$this->identifier.'='.$id.'&update'.$this->table.'&token='.($token!=NULL ? $token : $this->token).'">
						<img src="../img/admin/edit.gif" alt="" title="'.$_cacheLang['Edit'].'" /></a>';
					if ($this->delete AND (!isset($this->_listSkipDelete) OR !in_array($id, $this->_listSkipDelete)))
						echo '
						<!--<a class="action_delete" href="'.$currentIndex.'&'.$this->identifier.'='.$id.'&delete'.$this->table.'&token='.($token!=NULL ? $token : $this->token).'" onclick="return confirm(\''.$_cacheLang['DeleteItem'].$id.' ?\');" message="'.$_cacheLang['DeleteItem'].$id.' ?">-->
                        <a class="action_delete" href="'.$currentIndex.'&'.$this->identifier.'='.$id.'&delete'.$this->table.'&token='.($token!=NULL ? $token : $this->token).'" message="'.$_cacheLang['DeleteItem'].$id.' ?">
						<img src="../img/admin/delete.gif" alt="'.$_cacheLang['Delete'].'" title="'.$_cacheLang['Delete'].'" /></a>';
					$duplicate = $currentIndex.'&'.$this->identifier.'='.$id.'&duplicate'.$this->table;
					if ($this->duplicate)
						echo '
						<a class="pointer action_duplicate" onclick="if (confirm(\''.$_cacheLang['Copy images too?'].'\')) document.location = \''.$duplicate.'&token='.($token!=NULL ? $token : $this->token).'\'; else document.location = \''.$duplicate.'&noimage=1&token='.($token ? $token : $this->token).'\';">
        ...
        echo '<script type="text/javascript">
            jQuery(function(){
                ...
                {
                    case "AdminEmployees":
                        jQuery(".employee_active, .employee_Filter_active").css("display", "none");
                    break;
                }
	...
	public function displayListFooter($token = NULL)
	{
		echo '</table>';
		//if ($this->delete)
        if ($this->delete && $this->table == 'category'){
			//echo '<p style="float:left;"><input type="submit" class="button action_btn_del" name="submitDel'.$this->table.'" value="'.$this->l('Delete selection').'" onclick="return confirm(\''.$this->l('Delete selected items?', __CLASS__, TRUE, FALSE).'\');" /><p style="float:left;margin: 0 0 0 5px; padding:12px 0 5px 5px; width: 765px;">'.$this->l('When deleting Categories, all the products associated to the deletes Categories  will be deleted. You may meel to import the products.').'</p></p>';
            echo '<p style="float:left;"><input type="submit" class="button action_btn_del" name="submitDel'.$this->table.'" value="'.$this->l('Delete selection').'" message="'.$this->l('Delete selected items?', __CLASS__, TRUE, FALSE).'" /><p style="float:left;margin: 0 0 0 5px; padding:12px 0 5px 5px; width: 765px;">'.$this->l('When deleting Categories, all the products associated to the deletes Categories  will be deleted. You may meel to import the products.').'</p></p>';
        }else{
            //echo '<p><input type="submit" class="button action_btn_del" name="submitDel'.$this->table.'" value="'.$this->l('Delete selection').'" onclick="return confirm(\''.$this->l('Delete selected items?', __CLASS__, TRUE, FALSE).'\');" /></p>';
            echo '<p><input type="submit" class="button action_btn_del" name="submitDel'.$this->table.'" value="'.$this->l('Delete selection').'" message="'.$this->l('Delete selected items?', __CLASS__, TRUE, FALSE).'" /></p>';
        }
		echo '
				</td>
			</tr>
	...
	public function displayOptionsList()
	{
		...
		foreach ($this->_fieldsOptions AS $key => $field)
		{
			...
			echo '<div class="margin-form">
					<input type="submit" value="'.$this->l('   Save   ').'" name="submitOptions'.$this->table.'" class="button action_submit_form" />
				</div>
			</fieldset>
	...
	public function displayForm($isMainTab = true)
	{
        ...
        $lang_default = intval(Configuration::get('PS_VIEW_FORMULARIO_LANG'));
        //echo "aaa: ".$lang_default;
        //$langdefault = intval(Configuration::get('PS_LANG_DEFAULT'));
        //echo "ddd: ".$this->_defaultFormLanguage;
        ...
        if($table == "employee")
        {
            echo '
                <script type="text/javascript">
                    jQuery(function()
                    {
                        jQuery("#form_store_remote").submit(function()
                        {
                            var datos = jQuery(this).serialize();
                            var url = jQuery("#name_store_remote");
                            var valid = ValidaURL(url.val());

                            //if(valid)
                            //{
                                jQuery("#error_address_remote").css("display", "none");
                                jQuery.ajax({
                                    type: "post",
                                    url: "ajax.php",
                                    data: datos,
                            ...
                            //}else{
                                //jQuery("#error_address_remote").css("display", "block");
                            //}
                            return false;
                        });
                    });
            ...
            echo '
                ...
                <form id="form_store_remote" class="width2" onsubmit="return false;">
                    ...
                    <p style="color:#7F7F7F;">'.$this->l("Ejemplo:").' http://www.team-web.es/T-90/admin22</p>        
                </form>';    
        } 
    }
    
    /**
     * method that delete messages
     * @param integer $id Object id used for delete messages
     * 
     */
     ...
     public function deleteMessageOrder($id)
     {
        ...
        $messagesOrder = Message::getMessagesOrder($id);
        
        if(count($messagesOrder) > 0)
        {
            foreach($messagesOrder as $message)
            {
                Message::deleteMessage($message["id_message"]);
            }
        }
